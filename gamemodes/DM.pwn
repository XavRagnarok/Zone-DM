	...
	switch(dialogid)
	{
	    case DIALOG_REGISTER:
	    {
			...
			if(strlen(inputtext) > 128 || strlen(inputtext) < 3)
			    return ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Registration page", "{ffffff}It seems that you are not {ff0000}Registered {ffffff}so kindly please register yourself\nby entering a new password in the space given below:\nthank you!", "Register", "Close");
...
function:LogPlayerIn(playerid)
{
	...
	if(!rows)
	{
        ...
        ShowPlayerDialog(playerid, DIALOG_REGISTER, DIALOG_STYLE_PASSWORD, "Welcome to Zone's DM server", "{ffffff}It seems that you are not {ff0000}Registered {ffffff}so kindly please register yourself\nby entering a new password in the space given below:\nthank you!\n\n", "Register", "Close");
		return 1;
	}

	SCMex(playerid, COLOR_YELLOW, "Welcome to Zones DM server!");
	ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Welcome to Zone's DM server", "{ffffff}It seems that you already have an account, so kindly please {ff0000}Login {ffffff}by entering your password in the given section below:\nThank you!\n\n", "Login", "Close");
	return 1;
}
...
function:LoggingIn(playerid)
{
	if(!cache_num_rows())
	{
		...
		return ShowPlayerDialog(playerid, DIALOG_LOGIN, DIALOG_STYLE_PASSWORD, "Welcome to Ragnarok's Scripting \tAdventure", "You entered the wrong password!\n\nSERVER: You can now login!\nTIP: Please report all bugs that you\nmay have found to development.\n\n           Enter Your Password:", "Login", "Cancel");
	}
